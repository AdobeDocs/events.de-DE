---
user-guide-title: Acrobat Sign-Webinare
user-guide-description: Erfahren Sie von Adobe-Experten, wie Sie neue E-Signatur-Workflows in Acrobat Sign beherrschen.
cloud: Document Cloud
solution: Document Cloud
source-git-commit: 80ce8dae288f75550ac9c967cf67f542bf5e6fce
workflow-type: tm+mt
source-wordcount: '130'
ht-degree: 5%

---


# Acrobat Sign-Webinare {#acrobat-sign-webinars}

+ [Überblick](overview.md)
+ Webinare veröffentlichen {#releases}
   + [Februar 2026](skill-builders/acrobat-sign-february-2026-release.md)
   + [Oktober 2025](skill-builders/acrobat-sign-october-2025-release.md)
   + [Juli 2025](skill-builders/acrobat-july-release.md)
+ Innovation - Highlights der neuen Funktionen {#newfeatures}
   + [Konfigurieren von für Gesundheitswesen, Biowissenschaften und FDA-Vorschriften](./skill-builders/regulated-best-practices.md)
   + [Erste Schritte mit benutzerdefinierten Workflows](./skill-builders/custom-workflows.md)
   + [Identitätsüberprüfung und Sicherheit](./skill-builders/identity-verification-security.md)
   + [Mobile E-Signatur optimieren](./skill-builders/mobile-e-signing.md)
   + [Web Forms](./skill-builders/web-forms.md)
+ 101 - Erste Schritte für Benutzer {#getstartedusers}
   + [Acrobat Sign 101](./skill-builders/getting-started.md)
   + [Authoring-Umgebung](./skill-builders/authoring-environment.md)
   + [Vorbereiten Ihrer Vereinbarungen auf die elektronische Signatur](./skill-builders/e-signature-setup.md)
   + [Einblick in den Status der Vereinbarung](./skill-builders/agreement-status.md)
   + [Arbeiten mit elektronischen Signaturen in Ihren Microsoft-Programmen](./skill-builders/e-signature-microsoft.md)
   + [Erstellen und Verwenden von Workflows](./skill-builders/create-use-workflows.md)
+ 101 - Erste Schritte für Administratoren {#getstartedadmins}
   + [Acrobat Sign Admin-Erlebnis einrichten](./skill-builders/acrobat-sign-admin-experience.md)
   + [Administratoren auf ETLA](./skill-builders/acrobat-sign-admin-etla.md)
   + [Erweiterte Berichterstellung](./skill-builders/advanced-reporting.md)
+ 102 - Erweiterte Aufgaben {#advancedtasks}
   + [Erweiterte Tipps zum Senden von Dokumenten zur Signatur](./skill-builders/advanced-sending-documents-signature.md)
   + [Sammeln vieler Signaturen mit einem Klick](./skill-builders/collect-signatures.md)
   + [Document Builder](./skill-builders/document-builder.md)
   + [Posten eines ausfüllbaren, signierbaren Web-Formulars](./skill-builders/fillable-signable-web-form.md)
   + [Workflow-Automatisierungen](./skill-builders/workflow-automations.md)
